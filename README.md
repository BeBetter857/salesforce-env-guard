...
| 🔍 **一键自动检测** | 通过 Salesforce REST API 精确识别当前 Org 是否为生产环境，并可创建规则 |
| 🧭 **后台静默发现** | 打开 Salesforce 页面时自动记录未启用规则，在 Popup 中显示「自动发现」标记，启用后会清除「自动发现」标记 |
| 📋 **灵活规则管理** | 支持按域名精确匹配、规则搜索、拖拽排序优先级、启用/禁用 |
| 📦 **导入 / 导出** | 支持 JSON 备份和迁移规则，导入时会合并并跳过重复域名 |
...
2. 点击工具栏扩展图标，打开管理面板
3. 在「规则列表」中查看后台静默发现的规则；未启用的自动发现规则会带有「自动发现」标记
4. 启用需要保护的规则后，该规则会变成普通规则，并立即参与页面匹配
5. 也可以切换到「检测」标签，点击「开始检测」自动识别当前环境并创建规则
6. 或手动在「添加规则」标签中配置域名和环境类型
7. 规则较多时，可直接在规则列表顶部搜索规则名、域名或环境类型
8. 默认语言会按浏览器地区选择：中国显示中文、日本显示日本語、美国和其他国家显示 English；也可在「设置」中手动切换
9. 在「设置」中可导入 / 导出规则，方便备份或迁移
10. 此后每次打开对应的 Salesforce 页面，四周即显示对应颜色的边框
